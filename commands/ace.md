---
description: Manage ACE memory Playbook (status, search, config, health, setup, export, clear)
...
```
node "${CLAUDE_PLUGIN_ROOT}/dist/scripts/ace-cli.js" <subcommand> [args]
```
...
### setup
Downloads the ONNX embedding model (`all-MiniLM-L6-v2`) for semantic search. Only needs to run once. Sources: HuggingFace → hf-mirror → ModelScope (auto fallback). Progress is printed to stderr. On completion returns `{ "success": true }`.
